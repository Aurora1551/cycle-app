...
const OnboardingName: React.FC<OnboardingNameProps> = ({ onBack, onContinue, initialValue }) => {
  ...
  return (
    <div className="screen fade-in" style={{ background: '#FDF6F0', opacity: visible ? 1 : 0 }}>
      ...
      <button onClick={onBack} className="btn-back" style={{ color: '#9B7B74' }}>← back</button>

      <div className="content" style={{ padding: '16px 24px 32px', gap: 10 }}>
        <div className="step-label" style={{ color: '#C4614A', marginBottom: 2 }}>Step {STEP} of {TOTAL_STEPS}</div>
        <h1 className="heading" style={{ color: '#1C0F0C', marginBottom: 4 }}>What's your name?</h1>
        <p className="subtext" style={{ marginBottom: 8 }}>We'll use this to make everything feel personal.</p>
