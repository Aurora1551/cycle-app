    ...
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibeBg, display: 'flex', flexDirection: 'column', opacity: visible ? 1 : 0, transition: 'opacity 0.4s ease' }}>
      ...
      <div style={{ padding: '24px 24px 32px', display: 'flex', flexDirection: 'column', flex: 1, gap: 14 }}>
        ...
        <p style={{ fontFamily: "'Karla', sans-serif", fontSize: 13, fontWeight: 300, color: mutedColor, lineHeight: 1.5 }}>
          Your progress is saved — pick up exactly where you left off, on any device.
        </p>
        ...
        <button onClick={handleEmailSignUp} disabled={loading || !email || !password} style={{ width: '100%', background: email && password ? vibeAccent : `${vibeAccent}44`, color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontFamily: "'Karla', sans-serif", fontSize: 15, fontWeight: 600, cursor: email && password ? 'pointer' : 'default', letterSpacing: '0.02em', transition: 'background 0.2s ease' }}>
          {loading ? 'Saving...' : 'Save my journey →'}
        </button>
