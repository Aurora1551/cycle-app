...
const Summary: React.FC<Props> = ({ data, onStartFree, onUnlock }) => {
  ...
  const componentSummary = data.components
    .slice(0, 4)
    .map(c => COMPONENT_LABELS[c] || c)
    .join(' · ') + (data.components.length > 4 ? ` +${data.components.length - 4}` : '')
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '20px 24px 32px', display: 'flex', flexDirection: 'column', flex: 1, gap: 14 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#E8A598' }}>
          All set
        </div>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 34, fontWeight: 700, color: '#FDF6F0', lineHeight: 1.1, fontStyle: 'italic', margin: 0 }}>
          Ready for you,<br />
          <span style={{ color: '#E8A598' }}>{data.name}.</span>
        </h1>

        <p style={{ fontFamily: "'Karla', sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(253,246,240,0.4)' }}>
          Here's your journey
        </p>

        <div style={{ background: 'rgba(232,165,152,0.07)', border: '1px solid rgba(232,165,152,0.15)', borderRadius: 14, padding: '14px 16px' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#E8A598', marginBottom: 10 }}>
            ✦ Day 1 preview
          </div>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, fontStyle: 'italic', color: '#FDF6F0', lineHeight: 1.5, marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#FDF6F0', fontFamily: "'Karla', sans-serif" }}>Bichota</div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(232,165,152,0.1)', borderRadius: 14, padding: '4px 14px' }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 0', borderBottom: i < 3 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(232,165,152,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>
                {row.icon}
              </div>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#FDF6F0', fontFamily: "'Karla', sans-serif" }}>{row.val}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <button onClick={onUnlock} style={{ width: '100%', background: 'transparent', color: '#E8A598', border: '1px solid rgba(232,165,152,0.3)', borderRadius: 14, padding: '15px', fontFamily: "'Karla', sans-serif", fontSize: 14, fontWeight: 400, cursor: 'pointer' }}>
          Unlock full journey · from £5.99
        </button>

        <div style={{ textAlign: 'center', fontFamily: "'DM Mono', monospace", fontSize: 9, color: 'rgba(253,246,240,0.25)', letterSpacing: '0.08em' }}>
