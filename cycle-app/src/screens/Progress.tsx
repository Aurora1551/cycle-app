...
const Progress: React.FC<Props> = ({ data, dayNumber }) => {
  ...
  const handleDotClick = (day: number) => {
    ...
    if (isDone || day < dayNumber) {
      const stored = localStorage.getItem(`cycle_day_${day}`)
      if (stored) {
        try {
          setDayContent(JSON.parse(stored))
          setSelectedDay(day)
        } catch {}
      }
    }
  ...
  const spotifyUrl = dayContent
    ? `https://open.spotify.com/search/${encodeURIComponent(`${dayContent.songTitle} ${dayContent.songArtist}`)}`
    : ''

  return (
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibe.bg, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px 22px 100px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        {/* Dot grid */}
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: '18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase', color: vibe.accent }}>
            Day by day
          </div>
          {rows.map((row, wi) => (
            <div key={wi} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 7, color: mutedColor, width: 18, flexShrink: 0 }}>
                W{wi + 1}
              </div>
              {row.map(day => {
                const isDone = localStorage.getItem(`cycle_day_${day}_done`) === '1'
                const isCurrent = day === dayNumber
                const isPast = day < dayNumber || isDone
                return (
                  <button
                    key={day}
                    onClick={() => handleDotClick(day)}
                    title={isPast ? `Day ${day} — tap to view` : `Day ${day}`}
                    style={{
                      flex: 1,
                      aspectRatio: '1',
                      maxWidth: 34,
                  ...
                  >
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 7, color: isDone ? 'white' : isCurrent ? vibe.accent : mutedColor, lineHeight: 1 }}>
                      {day}
                    </span>
                  </button>
                )
          ...
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, color: mutedColor, letterSpacing: '0.08em' }}>
            Tap a completed day to revisit it
          </div>
        </div>

        {/* Stats */}
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: '18px' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase', color: vibe.accent, marginBottom: 14 }}>
            Stats
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            {[
              { val: daysCompleted, label: 'Days done' },
              { val: data.cycleDays, label: 'Total days' },
              { val: data.components.length, label: 'Components' },
            ].map((stat, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: typo.headingFont, fontSize: 30, fontWeight: typo.headingWeight, fontStyle: typo.headingStyle, color: vibe.accent, lineHeight: 1 }}>{stat.val}</div>
      ...
      {selectedDay !== null && dayContent && (
        <div
          onClick={() => { setSelectedDay(null); setDayContent(null) }}
        ...
        >
          <div
            ...
            style={{ width: '100%', maxWidth: 390, background: vibe.bg, borderRadius: '20px 20px 0 0', padding: '24px 24px 40px', maxHeight: '75vh', overflowY: 'auto' }}
          >
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: vibe.accent, marginBottom: 8 }}>
              Day {selectedDay}
            </div>
            <div style={{ fontFamily: typo.headingFont, fontStyle: 'italic', fontWeight: typo.headingWeight, fontSize: 18, color: textColor, lineHeight: 1.5, marginBottom: 6 }}>
              "{dayContent.quote}"
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 12, padding: '12px', marginBottom: 16 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: mutedColor, fontFamily: typo.bodyFont }}>{dayContent.songArtist}</div>
              </div>
            ...
            {dayContent.affirmation && (
              <div style={{ fontFamily: typo.headingFont, fontStyle: typo.headingStyle, fontSize: 16, fontWeight: typo.headingWeight, color: vibe.accent, textAlign: 'center', padding: '8px 0 16px' }}>
                {dayContent.affirmation}
              </div>
            )}
            <button
              onClick={() => { setSelectedDay(null); setDayContent(null) }}
              style={{ width: '100%', background: vibe.accent, color: 'white', border: 'none', borderRadius: 12, padding: '14px', fontFamily: typo.bodyFont, fontWeight: 600, fontSize: 14, cursor: 'pointer' }}
            >
              Close
            </button>
          </div>
