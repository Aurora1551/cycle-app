  ...
  return (
    <div className="fade-in w-full flex-center" style={{ opacity: visible ? 1 : 0 }}>
      <OnboardingLayout step={3} totalSteps={6} bg={BG} accent={A} text={T} muted={M} onBack={onBack}>
        <div className="step-label" style={{ color: A }}>Step 3 of 6</div>
        <h1 className="heading" style={{ color: T }}>How many days?</h1>
        <p className="subtext">We'll create a unique companion for each day.</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8 }}>
          ...
          <p className="subtext">days in<br />your cycle</p>
        </div>

        <div style={{ background: 'white', border: '1px solid rgba(196,97,74,0.12)', borderRadius: 12, padding: '14px 16px', marginTop: 4 }}>
          <div className="mono-hint" style={{ color: M, marginBottom: 10 }}>Your journey preview</div>
          ...
          <div style={{ fontFamily: "'Karla', sans-serif", fontSize: 11, color: M, marginTop: 8 }}>{filledDots} of {days} days shown · first 3 days free</div>
        </div>

        <div className="spacer" />
        <button onClick={() => onContinue(days)} className="btn-primary" style={{ background: A }}>Continue →</button>
      </OnboardingLayout>
    </div>
  )
}
