...
const GiftFlow: React.FC<Props> = ({ onBack, vibeAccent = '#C4614A', vibeBg = '#FDF6F0' }) => {
  ...
  const whatsappText = `Hey${recipientName ? ` ${recipientName}` : ''},\n\nI wanted to give you something that helped me through my fertility journey — a daily companion called Cycle.\n\nIt has daily quotes, meditations, a journal, music playlists and affirmations — all personalised for your treatment journey.${message ? `\n\nFrom me to you:\n${message}` : ''}\n\nYou don't have to go through this alone. 💛\n\n→ Start here: ${giftLink}`
  ...
  if (step === 'success') return (
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibeBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: 28, opacity: visible ? 1 : 0, transition: 'opacity 0.4s' }}>
      ...
      <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 700, color: textColor, textAlign: 'center', margin: 0, lineHeight: 1.2 }}>
        Gift sent with love
      </h2>
      ...
      <a
        ...
        style={{ display: 'block', width: '100%', background: '#25D366', color: 'white', border: 'none', borderRadius: 14, padding: '18px', fontFamily: "'Karla', sans-serif", fontSize: 16, fontWeight: 700, textAlign: 'center', textDecoration: 'none', letterSpacing: '0.02em' }}
      >
        Send via WhatsApp 💚
      </a>
      <button
        onClick={copyLink}
        style={{ width: '100%', background: 'transparent', color: copied ? vibeAccent : mutedColor, border: `1px solid ${cardBorder}`, borderRadius: 14, padding: '14px', fontFamily: "'Karla', sans-serif", fontSize: 14, cursor: 'pointer' }}
      >
        {copied ? '✓ Link copied!' : 'Copy gift link'}
      </button>
      <button
        onClick={onBack}
        style={{ background: 'none', border: 'none', fontFamily: "'DM Mono', monospace", fontSize: 11, color: mutedColor, cursor: 'pointer', letterSpacing: '0.08em' }}
      >
        Back to my day
      </button>
    </div>
  ...
  if (step === 'payment') return (
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibeBg, display: 'flex', flexDirection: 'column', opacity: visible ? 1 : 0, transition: 'opacity 0.4s ease' }}>
      <button onClick={() => setStep('compose')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: '0.1em', color: mutedColor, padding: '20px 24px 0', textAlign: 'left' }}>← back</button>
      <div style={{ padding: '20px 24px 32px', display: 'flex', flexDirection: 'column', flex: 1, gap: 16 }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 30, fontWeight: 700, color: textColor, lineHeight: 1.1, margin: 0 }}>
          Gift a Cycle <span style={{ color: vibeAccent }}>· £12.99</span>
        </h1>
        <p style={{ fontFamily: "'Karla', sans-serif", fontSize: 13, color: mutedColor, lineHeight: 1.5 }}>
          {recipientName ? `For ${recipientName} — ` : ''}full access to all days, personalised content, and Spotify links.
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button
            onClick={() => setStep('success')}
            style={{ width: '100%', background: '#000', color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontFamily: "'Karla', sans-serif", fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            <span style={{ fontSize: 18 }}>🍎</span> Pay with Apple Pay
          </button>
          <button
            onClick={() => setStep('success')}
            style={{ width: '100%', background: 'white', color: '#1a73e8', border: '2px solid #dadce0', borderRadius: 14, padding: '16px', fontFamily: "'Karla', sans-serif", fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            <span style={{ fontSize: 18 }}>G</span> Pay with Google Pay
          </button>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1, height: 1, background: cardBorder }} />
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: mutedColor, letterSpacing: '0.1em' }}>or pay by card</span>
          <div style={{ flex: 1, height: 1, background: cardBorder }} />
        </div>
        ...
        <button
          onClick={() => setStep('success')}
          style={{ width: '100%', background: vibeAccent, color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontFamily: "'Karla', sans-serif", fontSize: 15, fontWeight: 600, cursor: 'pointer', letterSpacing: '0.02em' }}
        >
          Pay £12.99 →
        </button>
        <div style={{ textAlign: 'center', fontFamily: "'DM Mono', monospace", fontSize: 8, color: mutedColor, letterSpacing: '0.08em' }}>
  ...
  return (
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibeBg, display: 'flex', flexDirection: 'column', opacity: visible ? 1 : 0, transition: 'opacity 0.4s ease' }}>
      <button onClick={onBack} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: '0.1em', color: mutedColor, padding: '20px 24px 0', textAlign: 'left' }}>← back</button>
      <div style={{ padding: '20px 24px 32px', display: 'flex', flexDirection: 'column', flex: 1, gap: 14 }}>
        ...
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 14, padding: '14px' }}>
          ...
          <div style={{ fontFamily: "'Karla', sans-serif", fontSize: 12, color: textColor, lineHeight: 1.6, whiteSpace: 'pre-line' }}>
            Hey{recipientName ? ` ${recipientName}` : ''},{'\n\n'}I wanted to give you something special — a daily companion called Cycle...
          </div>
        </div>
        <div style={{ flex: 1 }} />
        <button
          ...
          style={{ width: '100%', background: vibeAccent, color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontFamily: "'Karla', sans-serif", fontSize: 15, fontWeight: 600, cursor: 'pointer', letterSpacing: '0.02em' }}
        >
          Continue to payment · £12.99 →
        </button>
        <div style={{ textAlign: 'center', fontSize: 11, color: mutedColor }}>
          The most meaningful £12.99 you'll spend
        </div>
      </div>
    </div>
  )
