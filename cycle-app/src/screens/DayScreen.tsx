...
// Fixed component display order per spec
const COMPONENT_ORDER = ['quote', 'anthem', 'affirmation', 'journal', 'gratitude', 'meditation', 'breathing']
...
function BreathingCircle({ vibe, typo }: { vibe: typeof VIBES[0]; typo: ReturnType<typeof getTypo> }) {
...
function BoxBreathing({ vibe, typo }: { vibe: typeof VIBES[0]; typo: ReturnType<typeof getTypo> }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, padding: '16px 0' }}>
      ...
      <div style={{ fontFamily: typo.headingFont, fontStyle: typo.headingStyle, fontSize: 18, color: vibe.accent }}>
        {steps[step]}...
      </div>
    </div>
  )
...
function getTypo(vibeKey: string) {
  return VIBE_TYPO[vibeKey as keyof typeof VIBE_TYPO] || VIBE_TYPO.fierce
}
...
const DayScreen: React.FC<Props> = ({ data, dayNumber, isPremium, onDayComplete, onSettings }) => {
  ...
  const typo = getTypo(data.vibe || 'fierce')
  ...
  const markDone = () => {
    ...
    track('gratitude_saved', { day_number: dayNumber })
    setDayDone(true)
    onDayComplete()
  }

  const spotifyUrl = content
    ? `https://open.spotify.com/search/${encodeURIComponent(`${content.songTitle} ${content.songArtist}`)}`
    : ''
  ...
  const renderComponent = (component: string) => {
    if (component === 'quote') {
      ...
      return card(
        <>
          ...
          <div style={{ fontFamily: typo.headingFont, fontStyle: 'italic', fontWeight: typo.headingWeight, fontSize: 20, color: textColor, lineHeight: 1.4, marginBottom: 8 }}>
            "{content?.quote}"
          </div>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: vibe.accent, letterSpacing: '0.1em' }}>
            — {content?.quoteAuthor}
          </div>
        </>,
        component
    ...
    if (component === 'anthem') return card(
      <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <a
            ...
            onClick={() => track('song_played', { day_number: dayNumber })}
            style={{ background: '#1DB954', color: 'white', border: 'none', borderRadius: 20, padding: '7px 13px', fontSize: 11, fontWeight: 700, textDecoration: 'none', fontFamily: "'Karla', sans-serif", letterSpacing: '0.03em', whiteSpace: 'nowrap' }}
          >
            ▶ Spotify
          </a>
        </div>
    ...
    if (component === 'affirmation') return card(
      <>
        {sectionLabel('Affirmation')}
        <div style={{ fontFamily: typo.headingFont, fontStyle: typo.headingStyle, fontSize: 24, fontWeight: typo.headingWeight, color: vibe.accent, lineHeight: 1.3, textAlign: 'center', padding: '8px 0' }}>
          {content?.affirmation}
        </div>
      </>,
      component
    ...
    if (component === 'journal') return card(
      <>
        {sectionLabel('Journal')}
        <div style={{ fontSize: 14, color: textColor, fontFamily: typo.headingFont, fontStyle: 'italic', lineHeight: 1.4, marginBottom: 12 }}>
        ...
        <button
          onClick={saveJournal}
          style={{ marginTop: 8, background: journalSaved ? `${vibe.accent}20` : vibe.accent, color: journalSaved ? vibe.accent : 'white', border: journalSaved ? `1px solid ${vibe.accent}40` : 'none', borderRadius: 10, padding: '10px 18px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: typo.bodyFont, transition: 'all 0.2s' }}
        >
    ...
    if (component === 'gratitude') return card(
      <>
        ...
        <input
          ...
          style={{ ...inputStyle, minHeight: 'unset', resize: 'none' as const }}
        />
      </>,
      component
    )

    if (component === 'meditation') return card(
      <>
        {sectionLabel('Guided Meditation')}
        {!meditationStarted ? (
          <button
            onClick={() => { setMeditationStarted(true); track('meditation_started', { day_number: dayNumber }) }}
            style={{ width: '100%', background: `${vibe.accent}15`, border: `1px solid ${vibe.accent}30`, borderRadius: 12, padding: '20px', cursor: 'pointer', fontFamily: typo.bodyFont, fontWeight: 600, fontSize: 14, color: vibe.accent }}
          >
          ...
          <>
            ...
            <div style={{ textAlign: 'center', fontSize: 11, color: mutedColor, lineHeight: 1.5, fontFamily: typo.bodyFont }}>
    ...
    if (component === 'breathing') return card(
      <>
        {sectionLabel('Breathing Exercise')}
        <BoxBreathing vibe={vibe} typo={typo} />
        <div style={{ textAlign: 'center', fontSize: 11, color: mutedColor, fontFamily: typo.bodyFont }}>
    ...
    <div style={{ width: '100%', maxWidth: 390, minHeight: '100svh', background: vibe.bg, display: 'flex', flexDirection: 'column', opacity: visible ? 1 : 0, transition: 'opacity 0.5s ease' }}>
      <div style={{ padding: '20px 22px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <h1 style={{ fontFamily: typo.headingFont, fontSize: 28, fontWeight: typo.headingWeight, fontStyle: typo.headingStyle, color: textColor, margin: 0, lineHeight: 1 }}>
            Today
          </h1>
        </div>
        <button
          onClick={onSettings}
          style={{ background: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)', border: 'none', borderRadius: 10, width: 34, height: 34, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}
        >
          ⚙️
        </button>
      </div>
      ...
      {loading ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: typo.headingFont, fontStyle: 'italic', fontSize: 15, color: mutedColor }}>
            Preparing your day...
          </div>
          <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
        </div>
      ) : isLocked ? (
        /* Locked state for Day 4+ free users */
        <div style={{ padding: '0 18px 160px', display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
          {sortedComponents.slice(0, 2).map(component => (
            <div key={component} style={{ opacity: 0.35, filter: 'blur(2px)', pointerEvents: 'none' }}>
              {renderComponent(component)}
            </div>
          ))}
          <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: '28px 18px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <div style={{ fontSize: 32 }}>🔒</div>
            <div style={{ fontFamily: typo.headingFont, fontStyle: typo.headingStyle, fontWeight: typo.headingWeight, fontSize: 22, color: textColor, lineHeight: 1.2 }}>
              Day {dayNumber} is waiting for you
            </div>
            <div style={{ fontFamily: typo.bodyFont, fontWeight: typo.bodyWeight, fontSize: 13, color: mutedColor, lineHeight: 1.5 }}>
              3 free days done — unlock the rest of your journey
            </div>
            <button
              style={{ width: '100%', background: '#C4614A', color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontFamily: typo.bodyFont, fontWeight: 600, fontSize: 15, cursor: 'pointer', letterSpacing: '0.02em' }}
            >
        ...
        <div style={{ padding: '0 18px 160px', display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
          ...
          <button
            onClick={markDone}
            disabled={dayDone}
            style={{ width: '100%', background: dayDone ? `${vibe.accent}30` : vibe.accent, color: dayDone ? vibe.accent : 'white', border: dayDone ? `1px solid ${vibe.accent}50` : 'none', borderRadius: 14, padding: '16px', fontFamily: typo.bodyFont, fontWeight: 600, fontSize: 15, cursor: dayDone ? 'default' : 'pointer', letterSpacing: '0.02em', transition: 'all 0.3s', marginTop: 8 }}
          >
